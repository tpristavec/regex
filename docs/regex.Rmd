...
| Detect     | `grep`, `grepl`               | `str_detect`, `str_sub`   |
| Substitute | `gsub`, `sub`                 | `str_replace`, `str_sub`  |
...
.footnote[
The `+`, `*`, and `?` quantifiers are greed by default. This means they will match as many characters as possible. To make them lazy/reluctant, causing them to capture as few characters as possible, append a `?` (`+?`, `*?`, `??`).
]
...
| Symbol        | Function  
|---------------|----------------------------------------------------------------------------------|
| `(?=query)`   | + lookahead: "query" immediately follows the current position.                     |
| `(?!query)`   | - lookahead: "query" does _not_ immediately follow the current position.  |
| `(?<=query)`  | + lookbehind: "query" immediately precedes the current position.                   |
| `(?<!query)`  | - lookbehind: "query" does _not_ immediately precede the current position.|
...
`r countdown(minutes = 15, seconds = 00, play_sound = TRUE, font_size = "70px", color_text = "#23373B", color_background = "#FFFFFF", color_running_background = "#FFFFFF", color_running_text = "#23373B", color_finished_background = "#FFFFFF", color_finished_text = "#23373B", top = "50%", left = "35%")`
...
## Practice
[RegexOne lessons](https://regexone.com/), [FreeCodeCamp lessons](https://scrimba.com/g/gregularexpressions).

## Test query + canned regex
[Regex101](https://regex101.com/), [RegexR](https://regexr.com/), [Debuggex](https://www.debuggex.com/).

## Games
[The regex game](http://play.inginf.units.it/#/), [regex crossword](https://regexcrossword.com/).
